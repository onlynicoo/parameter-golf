...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Manually repeat KV heads for GQA (enable_gqa not supported on older GPUs)
...
def main() -> None:
    ...
    # BigramHash proj is a 2D matrix -> Muon
    if base_model.bigram_hash is not None and base_model.bigram_hash.proj is not None:
    ...
    # SmearGate gate -> scalar Adam
    if base_model.smeargate is not None:
        scalar_params.append(base_model.smeargate.gate)
    # BigramHash scale -> scalar Adam
    if base_model.bigram_hash is not None:
        scalar_params.append(base_model.bigram_hash.scale)
    token_lr = args.tied_embed_lr if args.tie_embeddings else args.embed_lr
    embed_params_list = [base_model.tok_emb.weight]
    # BigramHash embed -> embed Adam
    if base_model.bigram_hash is not None:
        embed_params_list.append(base_model.bigram_hash.embed.weight)
    ...
    log0(f"sdp_backends:cudnn=False flash={_use_flash} mem_efficient={not _use_flash} math={not _use_flash} amp_dtype={_amp_dtype}")
    ...
    log0(f"smeargate:{args.use_smeargate} bigram_hash_buckets:{args.bigram_hash_buckets} bigram_hash_dim:{args.bigram_hash_dim}")
    log0(f"ortho_init:{args.use_ortho_init} swa:{args.swa_enabled} swa_start_frac:{args.swa_start_frac} swa_every:{args.swa_every} prune_frac:{args.prune_frac}")
    ...
    while True:
        ...
        if args.swa_enabled and scale < args.swa_start_frac and step % args.swa_every == 0:
            sd = {k: v.detach().cpu().double() for k, v in base_model.state_dict().items()}
    ...
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        ...
        for k, v_sum in swa_state.items():
            avg_sd[k] = (v_sum / swa_count).to(dtype=current_sd[k].dtype)
    ...
    if args.prune_frac > 0:
        ...
        log0(f"magnitude_pruning: pruned {pruned_count}/{total_count} weights ({100*pruned_count/max(total_count,1):.1f}%)")
