...
# --- INT6 mixed quantization (numpy-based for MLX) ---
...
def quantize_int6_per_row_np(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
...
def mixed_quantize_int6(flat_state: dict[str, mx.array], int6_cats: set[str]):
    result: dict[str, np.ndarray] = {}
    meta: dict[str, object] = {}
    for name, arr in flat_state.items():
        cat = _classify_param(name)
        is_float = mx.issubdtype(arr.dtype, mx.floating)
        if not is_float or int(arr.size) <= 65536:
            result[name] = np.array(arr.astype(mx.float16) if is_float else arr, copy=True)
        ...
        if cat in int6_cats and arr.ndim >= 1:
            q, s = quantize_int6_per_row_np(arr)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def dequantize_mixed_int6(result: dict[str, np.ndarray], meta: dict[str, object],
                          template_state: dict[str, mx.array]) -> dict[str, mx.array]:
    out: dict[str, mx.array] = {}
    for name, orig in template_state.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            if np_arr.dtype == np.float16 and orig_dtype in (mx.float32, mx.bfloat16):
                out[name] = mx.array(np_arr, dtype=orig_dtype)
            else:
                out[name] = mx.array(np_arr, dtype=orig_dtype) if orig_dtype != mx.float32 else mx.array(np_arr)
            continue
        ...
        mx_dtype = MX_DTYPE_FROM_NAME.get(str(orig_dtype).split(".")[-1], mx.float32)
        out[name] = mx.array(out_arr, dtype=mx_dtype)
    return out
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(flat_state, {"mlp", "attn"})
    quant_raw = pickle.dumps({"w": quant_result, "m": quant_meta}, protocol=pickle.HIGHEST_PROTOCOL)
    if _COMPRESSOR == "zstd":
    ...
    log(f"serialized_model_int6_{_COMPRESSOR}:{quant_file_bytes} bytes")
    ...
    quant_state = pickle.loads(decompressed)
    quant_flat = dequantize_mixed_int6(quant_state["w"], quant_state["m"], flat_state)
